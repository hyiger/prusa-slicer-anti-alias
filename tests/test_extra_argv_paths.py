...
def test_extra_argv_paths_are_tolerated(tmp_path):
...
""", encoding="utf-8")
    ...
    extra = tmp_path / "extra.gcode.pp"
    extra.write_text("; ignored\n", encoding="utf-8")

    script = Path(__file__).resolve().parents[1] / "prusaslicer_anti_alias_z.py"
    p = subprocess.run([sys.executable, str(script), str(g), str(extra), "--stl", str(stl)], capture_output=True, text=True)
